...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        #self.peg = PosCNN(embed_dim,embed_dim,s = 1)
        ...
        assert weight_init in ('jax', 'jax_nlhb', 'moco', '')
        head_bias = -math.log(self.num_classes) if 'nlhb' in weight_init else 0.
        
        #named_apply(get_init_weights_vit(weight_init, head_bias), self)
...
class GazeNet_full(nn.Module):
    def __init__(self, backbone='swin_tiny_patch4_window7_224', pretrained=True):
        ...
        for i in range(len(self.depths)):
            ...
            patch_num = self.patch_embeds[-1].num_patches + 1 if i == len(embed_dims) - 1 else self.patch_embeds[
                -1].num_patches
            self.pos_drops.append(nn.Dropout(p=0.))
    ...
    def forward_features(self, x):
        ...
        for i in range(len(self.depths)):
            if i == 0:
                H,W = int(x.shape[2]/4),int(x.shape[3]/4)
            else:

                H,W = int(img_h/2 ** (i+2)),int(img_w/2**(i+2))
            x = self.patch_embeds[i](x)
            x = self.pos_drops[i](x)
            for j,blk in enumerate(self.backbone.layer_batch[i].blocks):
                x = blk(x)
                if j == 0:
...
class GazeNet(nn.Module):
    def __init__(self, backbone='swin_tiny_patch4_window7_224', pretrained=True):
        ...
        embed_dim = 96
        self.peg = PosCNN(embed_dim, embed_dim)
...
if __name__ == "__main__":
    ...
    model = GazeNet('swin_tiny_patch4_window7_224')
