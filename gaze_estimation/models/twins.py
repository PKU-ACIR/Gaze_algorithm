...
class GroupBlock(TimmBlock):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,init_values=None,
        ...
        super(GroupBlock, self).__init__(dim, num_heads, mlp_ratio, qkv_bias, drop, attn_drop, init_values,
                                         drop_path, act_layer, norm_layer)
        del self.attn
        if ws == 1:
...
class GazeNet(nn.Module):
    def __init__(self, pretrained=True):
        ...
        self.backbone = alt_gvt_small()
        self.backbone.load_state_dict(torch.load("models/alt_gvt_small.pth",
