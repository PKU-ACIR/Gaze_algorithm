...
def parse_args():
    ...
    # ethx-gaze
    parser.add_argument(
        '--gazeEthimage_dir', dest='gazeEthimage_dir', help='Directory path for gaze images.',
        default='/datasets/xgaze/Image', type=str)
    parser.add_argument(
        '--gazeEthlabel_dir', dest='gazeEthlabel_dir', help='Directory path for gaze labels.',
        default='/datasets/xgaze/Label', type=str)
...
if __name__ == '__main__':
    ...
    if data_set=="gaze360":
        ...
        model.cuda(gpu)
        dataset=datasets.Ethxgaze(args.gazeEthlabel_dir, args.gazeEthimage_dir, transformations, 180, 4)
        print('Loading data.')
        train_loader_gaze = DataLoader(
            dataset=dataset,
            batch_size=int(batch_size),
        ...
        summary_name = '{}_{}'.format('L2CS-ethxgaaze-', int(time.time()))
        output=os.path.join(output, summary_name)
        if not os.path.exists(output):
            os.makedirs(output)
